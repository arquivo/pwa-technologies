...
def printRulesOnfile(linesTobePrint,initURL)
    ...
    linesTobePrint.each do |aux| # if contains indexed pages
        ...
        unless $waybackMachine.nil?
            target_aux.write("|- \n")
            if isHTTP
                target_aux.write("|http://p58.arquivo.pt/#{aux[0..aux.index('?').to_i-1]} \n http://p62.arquivo.pt/#{aux[0..aux.index('?').to_i-1]}  ||  p58.#{$waybackMachine[i].split("http://").last}#{initURL}\n p58.#{$waybackMachine[i].split("http://").last}#{initURL.split("http://").last}\n p62.#{$waybackMachine[i].split("http://").last}#{initURL}\n p62.#{$waybackMachine[i].split("http://").last}#{initURL.split("http://").last}\n")
            else
                target_aux.write("|http://p58.arquivo.pt/#{aux[0..aux.index('?').to_i-1]} \n http://p62.arquivo.pt/#{aux[0..aux.index('?').to_i-1]}  || p58.#{$waybackMachine[i].split("http://").last}http://#{initURL}\n p58.#{$waybackMachine[i].split("http://").last}#{initURL}\n p62.#{$waybackMachine[    i].split("http://").last}http://#{initURL}\n p62.#{$waybackMachine[i].split("http://").last}#{initURL}\n")
            end
            i+=1
        end
    end

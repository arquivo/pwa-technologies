...
public class TopDocCollector extends HitCollector {
  ...
  public TopDocCollector(int numHits) {
    this(numHits, false, new HitQueue(numHits,false));
  }
  
  /** Construct to collect a given number of hits.
   * @param numHits the maximum number of hits to collect
   */
  public TopDocCollector(int numHits, boolean reverse) {
    this(numHits, reverse, new HitQueue(numHits,reverse));
  }

  TopDocCollector(int numHits, boolean reverse, PriorityQueue hq) {
  ...
  public void collect(int doc, float score) {
    //if (score > 0.0f) { Necessary to show all URLs with exacturl. It will bring more noise into results.
      totalHits++;
      if (!reverse) {
    	  ...
    	  if (hq.size() < numHits || score <= maxScore) {
    		  hq.insert(new ScoreDoc(doc, score));
    		  maxScore = ((ScoreDoc)hq.top()).score; // maintain maxScore
    	  }
      }
    //}
  }

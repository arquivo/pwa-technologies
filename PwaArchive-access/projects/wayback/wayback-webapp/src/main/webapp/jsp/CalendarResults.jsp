...
<table border="0" cellpadding="5" width="100%" class="mainSearchBanner" cellspacing="0">
   <tr>
      <td>
            <%= fmt.format("PathQueryClassic.searchedFor",URLDecoder.decode(searchString,"UTF-8")) %> <!-- TODO MC charset BUG -->
      </td>
      <td align="right">
...
<%
		  ...
		  for(int j = 0; j < partitionResults.size(); j++) {
			...
			String digestDiff = result.get(WaybackConstants.RESULT_DIGEST_DIFF);
                        if (digestDiff!=null) {
 				if (digestDiff.equals("true")) {
					updated=true;
				}
				else {
					updated=false;
				}
				replayUrl = results.resultToReplayId(result);
			}
			else {
				// Sets the replay URL to use ID or fallback to URL	
				if ( (replayUrl = results.resultToReplayId(result)) == null) 

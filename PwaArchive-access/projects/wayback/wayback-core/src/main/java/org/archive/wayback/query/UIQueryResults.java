/* UIQueryResults
 *
 * $Id: UIQueryResults.java 2119 2007-12-22 02:19:35Z bradtofel $
 *
 ...
 *
 * @author brad
 * @version $Date: 2007-12-22 02:19:35 +0000 (Sat, 22 Dec 2007) $, $Revision: 2119 $
 */
public class UIQueryResults extends UIResults {
	...
	public UIQueryResults(HttpServletRequest httpRequest, 
		...
		if (this.searchUrl==null) {
			this.searchUrl=results.getFirstResultUrl();
		}
		/* BUG 0000155 */
		...
		/* BUG 0000155 - in this case only on document is returned */
		if (wbRequest.get(WaybackConstants.REQUEST_EXACT_DATE)==null) {
			this.exactRequestedTimestamp=this.firstResultTimestamp;
		}
		else {
			this.exactRequestedTimestamp = Timestamp.parseAfter(wbRequest.get(WaybackConstants.REQUEST_EXACT_DATE));
		}
		/* BUG 0000155 */
	 ...
	 * @return URL string that will replay the specified Resource Result.
	 */
	public String resultToReplayId(SearchResult result) {
		return "id"+ result.get(WaybackConstants.REQUEST_DOC_ID)+"index"+ result.get(WaybackConstants.REQUEST_INDEX_ID);
	}
	/* BUG 0000155 */

...
const patchBrowser = async () => {
    // @ts-ignore
    const importMeta = (typeof document === 'undefined' ? new (require('u' + 'rl').URL)('file:' + __filename).href : (document.currentScript && document.currentScript.src || new URL('chunk-8de9033d.js', document.baseURI).href));
    if (importMeta !== '') {
...
function global0 () {
    ...
    // Setup platforms
    __chunk_2.setupPlatforms(win);
    // create the Ionic.config from raw config object (if it exists)
    // and convert Ionic.config into a ConfigApi that has a get() fn
    const configObj = Object.assign({}, __chunk_2.configFromSession(win), { persistConfig: false }, Ionic.config, __chunk_2.configFromURL(win));
    __chunk_2.config.reset(configObj);
    if (__chunk_2.config.getBoolean('persistConfig')) {
        __chunk_2.saveConfig(win, configObj);
    }
    // first see if the mode was set as an attribute on <html>
    // which could have been set by the user, or by prerendering
    // otherwise get the mode via config settings, and fallback to md
    Ionic.config = __chunk_2.config;
    Ionic.mode = mode = __chunk_2.config.get('mode', (doc.documentElement.getAttribute('mode')) || (__chunk_2.isPlatform(win, 'ios') ? 'ios' : 'md'));
    __chunk_2.config.set('mode', mode);
    doc.documentElement.setAttribute('mode', mode);
    doc.documentElement.classList.add(mode);
    if (__chunk_2.config.getBoolean('_testing')) {
        __chunk_2.config.set('animated', false);
    }

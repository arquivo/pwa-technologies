...
class BackButton {
    constructor(hostRef) {
        ...
        this.onClick = async (ev) => {
            ...
            return __chunk_5.openURL(this.defaultHref, ev, 'back');
        };
    }
    get backButtonIcon() {
        return this.icon != null ? this.icon : __chunk_2.config.get('backButtonIcon', 'arrow-back');
    }
    get backButtonText() {
        const defaultBackButtonText = this.mode === 'ios' ? 'Back' : null;
        return this.text != null ? this.text : __chunk_2.config.get('backButtonText', defaultBackButtonText);
    }
    get hasIconOnly() {
    ...
    render() {
        ...
        return (__chunk_1.h(__chunk_1.Host, { onClick: this.onClick, class: Object.assign({}, __chunk_5.createColorClasses(color), { [mode]: true, 'button': true, 'back-button-disabled': disabled, 'back-button-has-icon-only': hasIconOnly, 'ion-activatable': true, 'ion-focusable': true, 'show-back-button': showBackButton }) }, __chunk_1.h("button", { type: type, disabled: disabled, class: "button-native" }, __chunk_1.h("span", { class: "button-inner" }, backButtonIcon && __chunk_1.h("ion-icon", { icon: backButtonIcon, lazy: false }), backButtonText && __chunk_1.h("span", { class: "button-text" }, backButtonText)), mode === 'md' && __chunk_1.h("ion-ripple-effect", { type: this.rippleType }))));
    }

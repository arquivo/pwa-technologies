...
function importHardwareBackButton() {
    ...
    if (hardwareBackConfig) {
        import('./hardware-back-button-4b937266.js').then(module => module.startHardwareBackButton());
    }
}
function importStatusTap() {
    const statusTap = config.getBoolean('statusTap', isPlatform(window, 'hybrid'));
    if (statusTap) {
        import('./status-tap-fcd5899f.js').then(module => module.startStatusTap());
    }
}
function importFocusVisible() {
    import('./focus-visible-2e541376.js').then(module => module.startFocusVisible());
}
function importTapClick() {
    import('./tap-click-0b550e56.js').then(module => module.startTapClick(config));
}
function importInputShims() {
    const inputShims = config.getBoolean('inputShims', needInputShims());
    if (inputShims) {
        import('./input-shims-667d45fc.js').then(module => module.startInputShims(config));
    }
}

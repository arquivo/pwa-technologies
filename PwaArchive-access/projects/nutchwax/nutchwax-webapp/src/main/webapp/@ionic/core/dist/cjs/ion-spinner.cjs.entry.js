...
class Spinner {
    ...
    getName() {
        const name = this.name || __chunk_2.config.get('spinner');
    ...
    hostData() {
        const mode = __chunk_1.getIonMode(this);
        return {
            class: Object.assign({}, __chunk_5.createColorClasses(this.color), { [mode]: true, [`spinner-${this.getName()}`]: true, 'spinner-paused': !!this.paused || __chunk_2.config.getBoolean('_testing') })
        };
    }
    __stencil_render() {
...
function buildCircle(spinner, duration, index, total) {
    ...
    return (__chunk_1.h("svg", { viewBox: "0 0 64 64", style: data.style }, __chunk_1.h("circle", { transform: "translate(32,32)", r: data.r })));
}
function buildLine(spinner, duration, index, total) {
    const data = spinner.fn(duration, index, total);
    data.style['animation-duration'] = `${duration}ms`;
    return (__chunk_1.h("svg", { viewBox: "0 0 64 64", style: data.style }, __chunk_1.h("line", { transform: "translate(32,32)", y1: data.y1, y2: data.y2 })));
}

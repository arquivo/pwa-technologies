...
<jsp:include page="/header.html"/>
 ...
 <form name="search" action="searchTREC.jsp" method="get">
...
<%
  for (int i = 0; i < length; i++) {      // display the hits
    ...
    String docno = detail.getValue("DOCNO");   
    String arcname = detail.getValue("arcname");   
    arcname="/home/nutchwax/arcs/GOV1/uncompressed/"+arcname.substring(arcname.indexOf("/GOV1/")+6,arcname.length()-3);
...
*/
    ...
    <b><%=docno%></b>
    <b><a href=readFileTREC.jsp?file=<%=arcname%>&docno=<%=docno%>>See File</a></b>
    ...
    -
    <a href="searchTREC.jsp?<%=more%>"><i18n:message key="moreFrom"/>
     <%=hit.getDedupValue()%></a>
    <% } %>
    -
    <a href="explain.jsp?<%=id%>&query=<%=URLEncoder.encode(queryString, "UTF-8")%>">explain</a>
    -
    <a href="anchors.jsp?<%=id%>&query=<%=URLEncoder.encode(queryString, "UTF-8")%>">anchors</a>
    </small>
    <br><br>
<% } %>
<!--
...
<% 
  if (currentPage > 1) {
    ...
    String previousPageUrl = request.getContextPath() + "/searchTREC.jsp?" +
...
<%
  if (currentPage < pagesAvailable) {
    ...
    String nextPageUrl = request.getContextPath() + "/searchTREC.jsp?" +
      "query=" + htmlQueryString +
      "&start=" + nextPageStart +
      "&hitsPerPage=" + hitsPerPage +
      "&hitsPerDup=" + hitsPerDup +

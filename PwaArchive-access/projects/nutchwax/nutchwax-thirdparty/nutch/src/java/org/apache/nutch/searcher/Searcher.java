...
public interface Searcher extends Closeable {
  ...
  Hits search(Query query, int numHits, int searcherMaxHits, int maxHitsPerDup,
          String dedupField, String sortField, boolean reverse, PwaFunctionsWritable functions, int maxHitsPerVersion)
    throws IOException;
  
  /** Return an HTML-formatted explanation of how a query scored. */
  String getExplanation(Query query, Hit hit) throws IOException;
  
  String getExplanation(Query query, Hit hit, PwaFunctionsWritable functions) throws IOException;
}

...
public class QueryFilters {
  ...
  /**
   * TODO MC
   * Set ranking at runtime at BasicQueryFilter 
   * @param bparams ranking parameters
   */
  /* TODO MC remove
  public void setDynamicBoosts(ArquivoWebWritable bparams) throws QueryException {
	  if (bparams==null) {
		  return;
	  }
	  
	  String methodsNames[]={"setUrlBoost","setAnchorBoost","setContentBoost","setTitleBoost","setHostBoost","setPhraseBoost"};
	  Class filterClass=null;	  
	  
	  for (int i = 0; i < this.queryFilters.length; i++) {		  
		  // using reflection to call basic filter plugin
		  filterClass=queryFilters[i].getClass();
		  if (filterClass.getName().equals("org.apache.nutch.searcher.basic.BasicQueryFilter")) { 
			  ...
			  try {			
				  ...
				  for (int j=0;j<methodsNames.length;j++) {					 
					  switch(j) {
					  case 0: faux=bparams.getUrlBoost(); break;
					  case 1: faux=bparams.getAnchorBoost(); break;
					  case 2: faux=bparams.getContentBoost(); break;
					  case 3: faux=bparams.getTitleBoost(); break;
					  case 4: faux=bparams.getHostBoost(); break;
					  case 5: faux=bparams.getPhraseBoost(); break;
					  }
					  
					  maux=filterClass.getMethod(methodsNames[j], new Class[] { float.class });	    		  
					  maux.invoke(queryFilters[i], new Object[] { faux });	    		 					  
				  }
				  int slop=bparams.getSlop();  	  
				  maux=filterClass.getMethod("setSlop", new Class[] { int.class });	    		  
				  maux.invoke(queryFilters[i], new Object[] { slop });	    		 				  				
			  }
			  catch(NoSuchMethodException e) {
				  throw new QueryException("Dynamic boosts not set because "+e.getMessage());
			  }
	  ...
	  throw new QueryException("Dynamic boosts not set because BasicQueryFilter class not found.");
  }
  */
}

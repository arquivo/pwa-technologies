...
import org.apache.lucene.search.queries.PwaSearchTypesQuery;
...
public class BasicQueryFilter implements QueryFilter {
  ...
  private static int SLOP = 3; // TODO MC - http://wiki.apache.org/lucene-java/LuceneFAQ#head-f9be4862880b672356a1e8da58931a3bff720c5d
  ...
  private static String[] includeExtensions = null; //TODO MC read from wax-default.xml
  private final static String EXACTURL_FIELD="exacturl";
  private final static String EXACTURLEXPAND_FIELD="exacturlexpand";
  ...
  /* TODO MC - ranking */  
  /** Set the boost factor for content matches. */
  public void setContentBoost(float boost) { FIELD_BOOSTS[CONTENT_BOOST] = boost; }

  /** Set the boost factor for title matches. */
  public void setTitleBoost(float boost) { FIELD_BOOSTS[TITLE_BOOST] = boost; }
  
  /** Set the boost factor for host matches. */
  public void setHostBoost(float boost) { FIELD_BOOSTS[HOST_BOOST] = boost; }
  /* TODO MC - ranking */
  ...
  private void addSloppyPhrases(Query input, BooleanQuery output) {
    ...
    for (int f = 0; f < FIELDS.length; f++) {
      ...
      sloppyPhrase.setSlop(/*"anchor".equals(FIELDS[f])  TODO MC - I commented because doesn't make any sense
   ...
   */
  private void addTermsNewModel(Query input, BooleanQuery output) {
	  Clause[] clauses = input.getClauses();
	  for (int i=0; i<clauses.length; i++) {
		  Clause c = clauses[i];
		  ...
		  for (int j=0; j<FIELDS.length; j++) {		
			  if (c.isPhrase()) {				  				  
				  ...
				  for (int k=0; k<terms.length; k++) {
					  exactPhrase.add(luceneTerm(FIELDS[j], terms[k]));
				  }
				  out.add(exactPhrase, BooleanClause.Occur.SHOULD);
			  }
			  else {
				  out.add(new TermQuery(luceneTerm(FIELDS[j], c.getTerm())), BooleanClause.Occur.SHOULD);
			  }
		  }
		  output.add(out, (c.isProhibited()
				  ? BooleanClause.Occur.MUST_NOT
